...
# 2. CLIA 전용 로짓 집계 및 성능 모니터링 전략 클래스
class CLIAServerStrategy(fl.server.strategy.FedAvg):
    def aggregate_fit(
        self,
        server_round: int,
        ...
        # FedAvg 알고리즘을 통해 클라이언트들의 (5, 5) 클래스 로짓을 표본 가중 평균하여 글로벌 로짓 생성
        aggregated_params, aggregated_metrics = super().aggregate_fit(server_round, results, failures)
        
        if results:
            print(f"\n" + "="*65)
            print(f" [CLIA Server Round {server_round}] 글로벌 로짓 집계 및 성능 요약")
            print(f" ---------------------------------------------------------------")
            ...
            for idx, (_, res) in enumerate(results, 1):
                ...
                c_bytes = res.metrics.get("upload_bytes", 100)
                print(f"   - 클라이언트 #{idx}: 정확도 {c_acc:.2f}% | MSE: {c_loss:.4f} | MAE: {c_mae:.4f} (전송: {c_bytes:,} B)")
            ...
            if aggregated_metrics:
                ...
                if "total_upload_bytes" in aggregated_metrics:
                    ...
                    total_kb = total_b / 1024.0
                    print(f" ▶ 총 수신 데이터 용량 : {total_kb:.2f} KB ({total_b:,} Bytes) [초경량]")
                    # PIA 통신량(약 304 KB) 대비 절감률 안내
                    pia_est_bytes = len(results) * 103808
                    savings = (1 - total_b / pia_est_bytes) * 100.0
                    print(f" ▶ PIA 대비 통신량 절감률 : {savings:.2f}% (약 {pia_est_bytes // max(total_b, 1)}배 경량)")
            print(f"="*65 + "\n")
    ...
    def aggregate_evaluate(
        ...
        if results and aggregated_metrics and "accuracy" in aggregated_metrics:
            print(f" >> [CLIA Round {server_round} Eval] 글로벌 평가 정확도: {aggregated_metrics['accuracy']:.2f}% | Loss: {loss:.4f}")
            
        return loss, aggregated_metrics

strategy = CLIAServerStrategy(
...
if __name__ == "__main__":
    ...
    print(f"=========================================================")
    print(f" [CLIA Server] PFL Logit Aggregation Server Started on Port {port} ")
    print(f" [CLIA Server] Waiting for {NUM_CLIENTS} distributed clients to connect... ")
    print(f"=========================================================")
