...
# Usage: python client_CLIA.py <server_address> [client_id] [data_path] [total_partitions]
server_address = sys.argv[1] if len(sys.argv) > 1 else "127.0.0.1:8081"
...
print(f"[Client-CLIA #{client_id}] Loading dataset from '{data_path}'...")
...
if total_partitions > 1:
    ...
    print(f"[Client-CLIA #{client_id}] Allocated partition {idx+1}/{total_partitions}: rows {start_idx}~{end_idx} ({len(df)} samples)")
...
class CLIAHealthClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        print(f"\n" + "="*65)
        print(f" [Client-CLIA #{self.client_id} | Round {current_round}] 로컬 학습 및 로짓 전송 요약")
        print(f" ---------------------------------------------------------------")
    ...
    def evaluate(self, parameters, config):
        ...
        mse, mae, accuracy = self.compute_metrics()
        current_round = config.get("server_round", self.round)
        print(f" >> [Client-CLIA #{self.client_id} | Round {current_round} Eval] Test Loss(MSE): {mse:.4f} | Accuracy: {accuracy:.2f}%")
...
if __name__ == "__main__":
    print(f"[Client-CLIA #{client_id}] Connecting to CLIA Server at {server_address}...")
    fl.client.start_client(
        server_address=server_address,
        client=CLIAHealthClient(client_id=client_id).to_client()
    )
